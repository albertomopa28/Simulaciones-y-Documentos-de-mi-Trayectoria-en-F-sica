...
f = (0:n-1) * df;
fmaxg = 50 * 10^6;
nmf = min(round(fmaxg / df), n); % Ajustar nmf para que no supere el tamaño de la señal
...
ejes(3) = ampmin;
hold;
stem(fteor * 1e-6, ampteor, 'r'); % Añadir las frecuencias teóricas en rojo
axis([f(5) * 1e-6, f(nmf) * 1e-6, ejes(3), ejes(4)]); % Ajustar los ejes
hold off;
...
xlabel('Frecuencia (MHz)');
ylabel('|E(f)|');
stem(fteor * 1e-6, ampteor, 'g'); % Añadir las frecuencias teóricas en verde
legend('FFT E1', 'FFT E2', 'Frecuencias teóricas'); % Actualizar la leyenda
ejes = axis;
ampmax = ejes(4);
ampmin = -0.1 * ampmax;
ejes(3) = ampmin;
axis([f(5) * 1e-6, f(nmf) * 1e-6, ejes(3), ampmax]); % Ajustar los ejes
hold off;
