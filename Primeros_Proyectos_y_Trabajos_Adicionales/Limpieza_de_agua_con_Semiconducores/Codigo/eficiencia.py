...
#moivilidad DE HUECOS tambien se busca en internet 
...
t_recombinacion=10**(-7)  #tiempo de recombinacion antes de llegar a la superficie
t_reaccion=250*10**(-9)   #tiempo de reaccion
E_B=6.284605524119999*10**(-19)    #Energia de la barrera en J aunque sea negativa ponerla positiva
E_gap=1.4393856945600002*10**(-19)         #E gab en J                                                
n=4                 #Indice de refraccion?
movlidad=2*0.39*10**(-2)    #movilidad huecos m2 V-1s-
nombre_archivo='Ge30.txt'     #Archivo que se va a leer
W=30*10**(-9)     #Es donde empieza el potencial de la barrera en m (dato sacado del paper 100 nm) lo he cambiado a 1000nm
...
def encontrar_intervalo(datos, tipo, margen=0.1):
    ...
    while np.sum(indices_intervalo) < 3:   #Numero de pumtos que usamos para el ajuste
        margen += 0.05
        indices_intervalo = (k >= k_extremo - margen) & (k <= k_extremo + margen)
...
def generate_photons_with_probability_and_recombination(temperature, num_photons, wavelength_min=1e-9, wavelength_max=3e-6):
        ...
        if h*c/chosen_wavelength-E_gap<0:
            ...
            if np.random.rand() > generation_probability:
                ...
                if np.random.rand() < recombination_prob:
                    ...
                    if bucle_temporal(t_recombinacion,t_reaccion)==0:
                        #print('No reacciona')
                        NR=NR+1
                    else:
    ...
    return np.array(photon_wavelengths)

# Parámetros
temperature = 5800  # Temperatura del cuerpo negro en Kelvin (aproximadamente la del sol)
num_photons = 1000  # Número que quieres que se absorban pensad que la eficiencia es muy pequeña




# Generar fotones con la probabilidad de generación adicional y recombinación
photon_wavelengths = generate_photons_with_probability_and_recombination(temperature, num_photons)
