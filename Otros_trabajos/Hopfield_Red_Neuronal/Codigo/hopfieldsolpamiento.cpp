...
#define mu 4 // número de partones almacenados
...
int main()
{
    ...
    double T, random, p, DeltaE,DeltaT;
    int S[N][N];
    double a[mu], theta[N][N], solapamiento[mu],solapamiento1[mu];
    ...
    T=0.0;
    DeltaT=0.0005;            // Elegimos el paso de la temperatura del sistema
    ...
    iteraciones = 25*N*N; // número de saltos que vamos a dar
    ...
    for (i = paso; i <= iteraciones; i++)
    {
        ...
        if ((i / paso) > 20) 
        {
            ...
            for (j = 0; j < (mu); j++)
            {
                solapamiento1[j]=solapamiento1[j]+solapamiento[j]/5;
            }
        }
